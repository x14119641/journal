<template> 
  <div class="">
    <form @submit.prevent="onSubmit" class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4">
      ...
      <button
        type="submit"
        class="min-w-32 bg-lime-400 text-white py-2 px-4 rounded hover:bg-lime-500 focus:outline-none focus:ring-2 focus:ring-lime-400"
      >
        Add
      </button>
    </form>
...
const onSubmit = async () => {
  ...
  if (amount.value > 0) {
    try {
      await portfolioStore.addFunds(amount.value); 
    } catch (error) {
      if (error.response) {
