...
async def add_transaction(transaction:Transaction,current_user:Annotated[UserLogin, Depends(get_current_active_user)],db:Database=Depends(get_db) ):
    result = await db.fetchone("SELECT add_transaction(($1),($2),($3),($4),($5),($6))", 
                        current_user.id, transaction.ticker, transaction.price, transaction.quantity, transaction.transaction_type,transaction.fee, transaction.details)
    print('Result: ', result)
    print(current_user.id, transaction.ticker, transaction.price, transaction.quantity, transaction.transaction_type,transaction.fee, transaction.details)
